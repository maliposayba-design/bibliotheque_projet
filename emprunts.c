...
int verifierstock(int isbn_recherche){
      ...
      while (fread(&livre_courant, sizeof(Livre), 1, f_books) == 1) {
            if (strcmp(livre_courant.isbn, isbn_recherche) == 0) {
                livre_trouve = 1;
                  if (livre_courant.stock <= 0) {
                    printf("[Alerte] Le livre '%s' est �puis� (Stock : 0).\n", livre_courant.titre);
                    fclose(f_books);
                    return;
                }
        ...
        if (!livre_trouve) {
            printf("[Erreur] Aucun livre trouv� avec l'ISBN : %s\n", isbn_recherche);
            fclose(f_books);
            return;
        }
        livre_courant.stock--;
...
void AjouterEmprunt(){
    ...
    if (U.id == -1) {
        printf("Erreur : Utilisateur non trouv� !\n");
        return;
    }
    L = trouverLivreParId(idLiv);
    if (L.id == -1) {
        printf("Erreur : Livre non trouv� !\n");
        return;
    }
    if (strcmp(U.etat, "BLOQUE") == 0) {
        printf("Erreur : Votre compte est bloqu� !\n");
        return;
    }
